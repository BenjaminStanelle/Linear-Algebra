...
def applyLS(t, bhat) :
    ...
    B0, B1x = np.linalg.lstsq(A, bhat, rcond=None)[0]
    x_lin=[B0, B1x]
    Ax= (x_lin[1]*t) + x_lin[0]
    norm_lin=np.linalg.norm(bhat-Ax)
    column3=np.square(t)
    A_sqrt = np.vstack([np.ones(len(t)),t,column3]).T 
    xquad = np.linalg.lstsq(A_sqrt, bhat, rcond=None)[0]
    yquad = (xquad[2]*column3) + (xquad[1]*t) + xquad[0]
    norm_line3 = np.linalg.norm(bhat - yquad)
    
    return x_lin, norm_lin, xquad, norm_line3
